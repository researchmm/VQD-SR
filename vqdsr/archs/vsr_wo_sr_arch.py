...
class MSRSWVSR_WO_SR(nn.Module):
    """
    ...
    def __init__(self, num_feat=64, num_block=(5, 3, 2), netscale=4):
        super(MSRSWVSR_WO_SR, self).__init__()
        self.num_feat = num_feat

        # 3(img channel) * 3(prev cur nxt 3 imgs) + 3(hr img channel) * netscale * netscale + num_feat
        self.recurrent_cell = RightAlignMSConvResidualBlocks(3 * 3 + num_feat, num_feat,
    ...
    def forward(self, x):
        ...
        for i in range(n):
            if i == 0:
                ...
                out, state = self.cell(torch.cat((x[:, i], x[:, i], x[:, i + 1]), dim=1), state)
            elif i == n - 1:
                # there is no next frame for the last frame, so reuse last frame as next
                out, state = self.cell(torch.cat((x[:, i - 1], x[:, i], x[:, i]), dim=1), state)
            else:
                out, state = self.cell(torch.cat((x[:, i - 1], x[:, i], x[:, i + 1]), dim=1), state)
            out_l.append(out)
