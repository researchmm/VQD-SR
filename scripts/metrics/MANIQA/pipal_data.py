...
class NTIRE2022_interval(torch.utils.data.Dataset):

    def __init__(self, ref_path, dis_path, transform, interval = 10):
        super(NTIRE2022, self).__init__()
        self.ref_path = ref_path
        self.dis_path = dis_path
        self.transform = transform
        ...
        ref_files_data, dis_files_data = [], []
        img_list = sorted(os.listdir(dis_path))
        for idx, dis in enumerate(img_list):
            ref = dis
            if (idx+1)%interval == 0:
                ref_files_data.append(ref)
                dis_files_data.append(dis)
