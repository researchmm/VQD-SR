    ...
    def run(self):
        while True:
            ...
            if self.args.outscale != self.args.netscale:
                ...
                out_img = cv2.resize(
                    out_img, (int(
                        w * self.args.outscale / self.args.netscale), int(h * self.args.outscale / self.args.netscale)),   # net can only output netscale, if want other outscale, resize for it by *out_scale/netscale
